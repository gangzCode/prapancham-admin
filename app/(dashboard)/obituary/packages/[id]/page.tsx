...
export default function EditPackagePage() {
    ...
    useEffect(() => {
        const fetchPackageData = async () => {
            ...
            try {
                ...
                form.reset({
                    name: packageData.name.en[0]?.value || "",
                    price: packageData.basePrice?.price || 0,
                    duration: packageData.duration?.toString() || "",
                    wordLimit: packageData.wordLimit || 0,
                    isObituary: packageData.isObituary || false,
                    isRemembarace: packageData.isRemembarace || false,
                    isPremium: packageData.isPremium || false,
                    isPriority: packageData.isPriority || false,
                    isFeatured: packageData.isFeatured || false,
                    isTributeVideoUploading: packageData.isTributeVideoUploading || false,
                    isAdditionalImages: packageData.isAdditionalImages || false,
                    isActive: packageData.isActive !== undefined ? packageData.isActive : true,
                    noofAdditionalImages: packageData.noofAdditionalImages || 0,
                    noofContectDetails: packageData.noofContectDetails || 0,
                    noofBgColors: packageData.noofBgColors || 0,
                    noofPrimaryImageBgFrames: packageData.noofPrimaryImageBgFrames || 0,
                    addons: packageData.addons?.map((addon: any) => addon._id) || [],
                    bgColors: packageData.bgColors?.map((color: any) => color.colorCode) || [],
                    primaryImageBgFrames: packageData.primaryImageBgFrames?.map((frame: any) => frame._id) || [],
                    description: {
                        en: packageData.description?.en || [{ name: "", value: "" }],
                        ta: packageData.description?.ta || [{ name: "", value: "" }],
                        si: packageData.description?.si || [{ name: "", value: "" }],
                    },
                    priceList: packageData.priceList?.map((item: any) => ({
                        country: item.country._id,
                        countrySpecificPrice: item.price
                    })) || [{ country: "", countrySpecificPrice: 0 }],
                })
    ...
    async function onSubmit(data: PackageFormValues) {
        ...
        try {
            ...
            toast({
                title: "Error",
                description: error instanceof Error ? error.message : "There was an error updating the package. Please try again.",
                variant: "destructive",
            })
        } finally {
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title={"Edit Package"}
                description="Edit obituary or remembrance package"
                href="/obituary/packages"
            />

            {(isLoadingPackage || packageNotFound) ? (
                <Card>
                    <CardContent className="pt-6">
                        <div className="text-center py-8">
                            {packageNotFound ? (
                                <div>
                                    <div className="text-lg text-destructive">Package not found</div>
                                    <p className="text-muted-foreground mt-2">The requested package could not be found.</p>
                                </div>
                            ) : (
                                <div className="text-lg">Loading package data...</div>
                            )}
                        </div>
                    </CardContent>
                </Card>
                            ...
                            <div className="border-t pt-6">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                                            <div className="space-y-0.5">
                                                <FormLabel className="text-base">Package Status</FormLabel>
                                                <FormDescription>
                                                    {field.value ? "Package is active and available for users" : "Package is inactive and not available for users"}
                                                </FormDescription>
                                            </div>
                                            <FormControl>
                                                <Switch 
